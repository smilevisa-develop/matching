    ...
    <Wrapper className={embedded ? "" : "rounded-2xl border border-gray-200 bg-white p-5 shadow-sm"}>
      ...
      {deliveries.length > 0 ? (
        <div className="mt-4 border-t border-gray-100 pt-3">
          ...
          >
            送信履歴（{deliveries.length}件）
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" className={`transition-transform ${showHistory ? "rotate-180" : ""}`} aria-hidden><polyline points="6 9 12 15 18 9" /></svg>
          </button>
          {showHistory ? (
